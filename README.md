...
\*\*\* **Lab 8** takes up exactly where Lab 7 stopped, and tests its diagnosis: **M0 (2026-08-15)** re-actuates the G1 with torque motors instead of position servos and re-establishes a 10 s stand from outside the simulator (0.71 mm CoM drift, model parity at 1e-16). **M1** adds the whole-body inverse-dynamics QP — joint accelerations *and* contact wrenches as decision variables — so the standing robot tracks a hand circle to 7.08 mm RMS while balance stays a hard constraint. **M2** takes four torque-level steps in place with the ZMP inside the support polygon 98.7 % of the time, after finding that commanding CoM *height* was saturating the waist actuators. 62 tests. Walking is this lab's own deliverable (M2–M3) rather than an import. See [`lab-8-loco-manipulation/`](lab-8-loco-manipulation/).
