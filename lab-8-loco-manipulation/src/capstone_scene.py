...
* a **weld** equality constraint between the right wrist and the payload,
  built inactive and switched on at the grasp — the "grasp stays SIMPLE"
  instruction in `plan/LAB_08.md`, and the same mechanism Lab 6 used for its
  cooperative carry.
...
PEDESTAL_Y = -0.45
PAYLOAD_INSET = 0.09        # payload offset from the pedestal centre, toward the robot
...
class CapstoneScene:
    ...
    def set_weld(self, active: bool) -> None:
        ...
        """
        if active:
            self._capture_relative_pose()
        self.data.eq_active[self.weld_id] = int(active)
        mujoco.mj_forward(self.model, self.data)

    def _capture_relative_pose(self) -> None:
        ...
        """
        hand = self.model.eq_obj1id[self.weld_id]
        payload = self.model.eq_obj2id[self.weld_id]
        ...
        self.model.eq_data[self.weld_id, 3:6] = relative_pos
...
def build_capstone_scene(
    ...
    def add_pedestal(name: str, x: float, top: float) -> None:
        body = spec.worldbody.add_body()
        body.name = name
        body.pos = [x, pedestal_y, 0.0]
        geom = body.add_geom()
        geom.name = f"{name}_geom"
        geom.type = mujoco.mjtGeom.mjGEOM_BOX
        geom.size = [PEDESTAL_HALF_XY, PEDESTAL_HALF_XY, top / 2.0]
    ...
    add_pedestal("pick_pedestal", pick_x, PEDESTAL_TOP)
    add_pedestal("place_pedestal", place_x, PEDESTAL_TOP)
    ...
    if payload_body < 0 or weld_id < 0:
    ...
    return CapstoneScene(
        ...
        place_target=np.array(
            [place_x, pedestal_y + PAYLOAD_INSET, PEDESTAL_TOP + PAYLOAD_HALF],
            dtype=float,
        ),
    )
